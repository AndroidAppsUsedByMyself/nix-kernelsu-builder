_: {
  perSystem =
    ...
    let
      ...
      # 获取设备配置，但过滤掉可能引起问题的函数
      lineageBaseConfig = pkgs.lib.filterAttrs (n: v: !(pkgs.lib.isFunction v)) (
        pkgs.callPackage ./LineageOS_pstar.nix {
          inherit sources config pkgs fetchGooglePrebuiltClang;
        }
      );

      myOtherDevicesConfig = pkgs.lib.filterAttrs (n: v: !(pkgs.lib.isFunction v)) (
        pkgs.callPackage ./MyOtherDevices.nix {
          inherit sources config pkgs fetchGooglePrebuiltClang;
        }
      );
    in
    {
      kernelsu = pkgs.lib.mkMerge [
        {
